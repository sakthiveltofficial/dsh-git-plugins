...
import { approveEscalation, ESCALATION_TARGETS, validateEscalationArgs, type SandboxExecutionPolicy } from '@deepseek-ai/dsh-sandbox'
import { GitError, parseRemote, type GitPlatform } from '@dsh-git/core'
...
type RepoAction = 'clone' | 'init' | 'add' | 'remote_add' | 'branch' | 'checkout' | 'commit' | 'push' | 'pull' | 'stash' | 'tag' | 'merge' | 'rebase'
type InspectAction = 'diff' | 'blame' | 'log' | 'search_code' | 'search_files' | 'status'
type PrAction = 'create' | 'list' | 'merge' | 'comment'
type IssueAction = 'list' | 'create'
type CiAction = 'list' | 'get'
...
function workdirOf(path: string | undefined, exec: ToolRunContext): string {
  if (path) return path
...
async function repoContext(ctx: Context, exec: ToolRunContext, args: PlatformArgs): Promise<RepoCtx> {
...
async function confirmDestructive(ctx: Context, exec: ToolRunContext, toolName: string, what: string): Promise<void> {
  const approval = ctx.get('approval') as { request(req: ApprovalRequest): Promise<ApprovalOutcome> } | undefined
  if (!approval || !exec.agent) return // no approval stack: composition decides confinement elsewhere
...
interface SandboxKit {
  escalationModes: string[]
  schemaFields(): ParameterSchemaSpec
  resolvePolicy(toolName: string, args: { sandbox_permissions?: string; justification?: string }, exec: ToolRunContext): Promise<SandboxExecutionPolicy | undefined>
}

function sandboxKit(ctx: Context): SandboxKit {
  const shell = ctx.get('shell') as { sandboxMode?: string } | undefined
  const escalationModes = shell?.sandboxMode ? [...ESCALATION_TARGETS] : []
  const policyService = ctx.get('sandboxPolicy') as { resolve(opts?: { session?: unknown }): SandboxExecutionPolicy } | undefined
  return {
    escalationModes,
    schemaFields(): ParameterSchemaSpec {
    ...
    async resolvePolicy(toolName, args, exec) {
      ...
      if (escalationModes.length === 0) throw new GitError('sandbox_permissions is not available in this composition (no sandboxing shell mounted)', 'GIT_SANDBOX_DENIED')
      if (!standing) throw new GitError('no sandbox policy to escalate', 'GIT_SANDBOX_DENIED')
...
function gitResultCard(_args: unknown, result: { meta?: unknown }) {
  const meta = result.meta as GitToolValue | undefined
  if (!meta || !meta.ok) return undefined
  switch (meta.action) {
    case 'search_code': {
      const matches: { path: string; line: number; text: string }[] = Array.isArray(meta.data) ? (meta.data as unknown as { path: string; line: number; text: string }[]) : []
      ...
      for (const [path, lines] of byPath) files.push({ path, matches: lines })
      return { card: 'search' as const, shape: 'matches' as const, title: 'Code search', files, truncated: false, total: matches.length }
    }
    case 'list':
    case 'create':
    case 'get':
      return { card: 'generic' as const, title: meta.title, content: [{ type: 'text' as const, text: meta.text }] }
    default:
      return undefined
  }
}

function formatLog(entries: { hash: string; author: string; date: string; subject: string }[]): string {
...
export function apply(ctx: Context, config: { confirmDestructive?: boolean } = {}): void {
  ...
  ctx.tools.register(defineTool({
    ...
    output: {
      ...
      render: (_args, value) => [{ type: 'text', text: (value as GitToolValue).text }],
    },
    async execute(args, exec) {
      const a = args as RepoArgs
      ...
      const destructiveReason = (): string | null => {
        if (a.action === 'push' && a.force) return `force-push to ${a.branch ?? 'current branch'} in ${workdir}`
        if (a.action === 'merge' && !a.abort) return `merge ${a.branch ?? ''} into ${workdir}`
        if (a.action === 'rebase' && !a.abort && !a.continue) return `rebase in ${workdir}`
        if (a.action === 'tag' && a.subaction === 'delete') return `delete tag ${a.tag ?? ''} in ${workdir}`
        return null
      }
      const reason = destructiveReason()
      if (confirmDestructiveEnabled && reason) await confirmDestructive(ctx, exec, 'git_repo', reason)

      switch (a.action) {
        case 'clone': {
          if (!a.url) throw new GitError('git_repo clone requires `url`', 'GIT_INVALID_ARGS')
          if (!a.destination) throw new GitError('git_repo clone requires `destination`', 'GIT_INVALID_ARGS')
        ...
        case 'remote_add': {
          if (!a.remote_url) throw new GitError('git_repo remote_add requires `remote_url`', 'GIT_INVALID_ARGS')
        ...
        case 'checkout': {
          ...
          if (!ref) throw new GitError('git_repo checkout requires `ref` or `branch`', 'GIT_INVALID_ARGS')
        ...
        case 'commit': {
          if (!a.message) throw new GitError('git_repo commit requires `message`', 'GIT_INVALID_ARGS')
        ...
        case 'stash': {
          ...
          const sub = (a.subaction && stashActions.has(a.subaction) ? a.subaction : 'list') as 'list' | 'push' | 'pop' | 'apply' | 'drop'
        ...
        case 'tag': {
          ...
          const sub = (a.subaction && tagActions.has(a.subaction) ? a.subaction : 'list') as 'list' | 'create' | 'delete'
        ...
        case 'merge': {
          ...
          if (!a.branch) throw new GitError('git_repo merge requires `branch`', 'GIT_INVALID_ARGS')
        ...
        default:
          throw new GitError(`unknown git_repo action: ${String((a as any).action)}`, 'GIT_INVALID_ARGS')
      }
    },
  }))
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      switch (a.action) {
        ...
        case 'blame': {
          if (!a.file) throw new GitError('git_inspect blame requires `file`', 'GIT_INVALID_ARGS')
        ...
        case 'search_code': {
          if (!a.query) throw new GitError('git_inspect search_code requires `query`', 'GIT_INVALID_ARGS')
        ...
        case 'search_files': {
          if (!a.pattern) throw new GitError('git_inspect search_files requires `pattern`', 'GIT_INVALID_ARGS')
...
${body}` }
        ...
        default:
          throw new GitError(`unknown git_inspect action: ${String((a as any).action)}`, 'GIT_INVALID_ARGS')
      }
    },
  }))
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      switch (a.action) {
        case 'create': {
          if (!a.title || !a.head || !a.base) throw new GitError('git_pr create requires `title`, `head`, and `base`', 'GIT_INVALID_ARGS')
        ...
        case 'merge': {
          if (!a.number) throw new GitError('git_pr merge requires `number`', 'GIT_INVALID_ARGS')
          if (confirmDestructiveEnabled) await confirmDestructive(ctx, exec, 'git_pr', `merge PR #${a.number} in ${rc.owner}/${rc.repo}`)
          await prs.mergePullRequest({ ...rc, number: a.number, method: a.method, signal })
          return { action: 'merge', ok: true, text: `merged PR #${a.number}` }
        }
        case 'comment': {
          if (!a.number || !a.body) throw new GitError('git_pr comment requires `number` and `body`', 'GIT_INVALID_ARGS')
        ...
        default:
          throw new GitError(`unknown git_pr action: ${String((a as any).action)}`, 'GIT_INVALID_ARGS')
      }
    },
  }))
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      switch (a.action) {
        ...
        case 'create': {
          if (!a.title) throw new GitError('git_issues create requires `title`', 'GIT_INVALID_ARGS')
        ...
        default:
          throw new GitError(`unknown git_issues action: ${String((a as any).action)}`, 'GIT_INVALID_ARGS')
      }
    },
  }))
  ...
  ctx.tools.register(defineTool({
    ...
    presentResult: gitResultCard,
    async execute(args, exec) {
      const a = args as ReleaseArgs
      const rc = await repoContext(ctx, exec, a)
      const signal = exec.signal
      if (confirmDestructiveEnabled) await confirmDestructive(ctx, exec, 'git_release', `create release ${a.tag} in ${rc.owner}/${rc.repo}`)
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      switch (a.action) {
        ...
        case 'get': {
          if (!a.id) throw new GitError('git_ci get requires `id`', 'GIT_INVALID_ARGS')
        ...
        default:
          throw new GitError(`unknown git_ci action: ${String((a as any).action)}`, 'GIT_INVALID_ARGS')
      }
    },
  }))
}

interface PrArgs extends PlatformArgs {
  action: PrAction
  title?: string
  head?: string
  base?: string
...
interface IssuesArgs extends PlatformArgs {
  action: IssueAction
  state?: 'open' | 'closed' | 'all'
  limit?: number
  title?: string
...
interface ReleaseArgs extends PlatformArgs {
  action: 'create'
  tag: string
  name?: string
  body?: string
...
interface SecurityArgs extends PlatformArgs {
  action: 'list'
}
interface CiArgs extends PlatformArgs {
  action: CiAction
  branch?: string
  limit?: number
  id?: string
