...
import { GitLocalService, GitError, } from '@dsh-git/core';
...
function classifyGitError(detail) {
    if (/not a git repository/i.test(detail))
        return 'GIT_NOT_A_REPO';
    if (/authentication failed|could not read username|permission denied \(publickey\)|invalid username or password/i.test(detail))
        return 'GIT_AUTH_FAILED';
    if (/conflict/i.test(detail))
        return 'GIT_CONFLICT';
    if (/please commit your changes or stash them|local changes would be overwritten|overwritten by merge/i.test(detail))
        return 'GIT_DIRTY';
    if (/rate limit|too many requests/i.test(detail))
        return 'GIT_RATE_LIMITED';
    if (/could not resolve host|connection (timed out|refused)|unable to access/i.test(detail))
        return 'GIT_NETWORK';
    return 'GIT_FAILED';
}
function hostOf(url) {
...
export class LocalGitService extends GitLocalService {
    ...
    fatal(result, subcmd) {
        if (result.aborted)
            throw new GitError('git command was aborted', 'GIT_ABORTED');
        if (result.timedOut)
            throw new GitError(`git ${subcmd} timed out after ${result.timeoutMs}ms`, 'GIT_TIMEOUT');
    ...
    async run(workdir, argv, opts = {}) {
        const result = await this.git(workdir, argv, opts);
        if (result.exitCode !== 0)
            this.fatal(result, argv[0]);
        return {
    ...
    async resolveHeadBranch(workdir, signal) {
        try {
    ...
    async clone(req) {
        if (!req.url)
            throw new GitError('clone requires a url', 'GIT_INVALID_ARGS');
        if (!req.destination)
            throw new GitError('clone requires a destination', 'GIT_INVALID_ARGS');
        const argv = ['clone'];
        if (req.depth !== undefined)
            argv.push('--depth', String(req.depth));
        if (req.branch)
            argv.push('--branch', req.branch);
    ...
    async checkout(req) {
        if (!req.ref)
            throw new GitError('checkout requires a ref', 'GIT_INVALID_ARGS');
        const argv = ['checkout'];
        if (req.force)
            argv.push('-f');
        if (req.create)
    ...
    async commit(req) {
        if (!req.message)
            throw new GitError('commit requires a message', 'GIT_INVALID_ARGS');
        const argv = ['commit', '-m', req.message];
        if (req.all)
            argv.push('-a');
        if (req.amend)
            argv.push('--amend', '--no-edit');
        const { stdout } = await this.run(req.workdir, argv, { signal: req.signal, sandboxPolicy: req.sandboxPolicy });
    ...
    async diff(req) {
        ...
        if (req.context !== undefined)
            argv.push(`--unified=${req.context}`);
        if (req.pathspec?.length)
            argv.push('--', ...req.pathspec);
        const { stdout, truncated } = await this.run(req.workdir, argv, { signal: req.signal });
        return { patch: stdout, truncated };
    }
    async blame(req) {
        if (!req.file)
            throw new GitError('blame requires a file', 'GIT_INVALID_ARGS');
    ...
    async tag(req) {
        ...
        if (!req.name)
            throw new GitError(`tag ${req.action} requires a name`, 'GIT_INVALID_ARGS');
        if (req.action === 'create') {
            const argv = ['tag'];
            if (req.message)
                argv.push('-a', req.name, '-m', req.message);
            else
                argv.push(req.name);
            if (req.ref)
                argv.push(req.ref);
    ...
    async stash(req) {
        ...
        if (req.action === 'push' && req.message)
            argv.push('-m', req.message);
        if (req.action !== 'push' && req.ref)
            argv.push(req.ref);
        const { stdout } = await this.run(req.workdir, argv, { signal: req.signal, sandboxPolicy: req.sandboxPolicy });
    ...
    async merge(req) {
        ...
        if (!req.branch)
            throw new GitError('merge requires a branch', 'GIT_INVALID_ARGS');
        const argv = ['merge'];
        if (req.noCommit)
            argv.push('--no-commit');
    ...
    async rebase(req) {
        ...
        if (result.exitCode !== 0)
            this.fatal(result, 'rebase');
        return { ok: true, summary: text };
    }
    async searchCode(req) {
        if (!req.query)
            throw new GitError('searchCode requires a query', 'GIT_INVALID_ARGS');
    ...
    async searchFiles(req) {
        if (!req.pattern)
            throw new GitError('searchFiles requires a pattern', 'GIT_INVALID_ARGS');
        const { stdout } = await this.run(req.workdir, ['ls-files', '--cached', '--others', '--exclude-standard'], { signal: req.signal });
