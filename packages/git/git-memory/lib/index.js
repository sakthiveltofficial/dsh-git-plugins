...
export class GitMemoryError extends Error {
    constructor(message, code = 'GIT_MEMORY_FAILED') {
        super(message);
...
function stableKey(kind, project, text) {
...
export class GitMemoryService extends Service {
    static { this.inject = ['storageDomain', 'systemPrompt']; }
    ...
    async open() {
        ...
        if (!storage)
            throw new GitMemoryError('storageDomain is not mounted; git memory is unavailable', 'GIT_MEMORY_NO_STORAGE');
    ...
    async record(input) {
        if (!this.config.enabled)
            throw new GitMemoryError('git memory recording is disabled', 'GIT_MEMORY_DISABLED');
        const table = await this.ensureTable();
    ...
    async recall(query = {}) {
        ...
        for (const [, entry] of table.entries()) {
            ...
            if (query.status && entry.status !== query.status)
                continue;
            if (query.project && entry.scope.project !== query.project)
                continue;
            if (needle && !entry.text.toLowerCase().includes(needle))
                continue;
            results.push(entry);
        }
    ...
    async observe(exec, result) {
        if (!this.config.enabled || !GIT_TOOLS.has(exec.name))
            return;
