...
export class GitError extends HarnessError {
  ...
  constructor(message: string, code: GitErrorCode, options?: ErrorOptions & { status?: number }) {
...
// ── HTTP plumbing (shared by adapters) ───────────────────────────────────────
 ...
 */
export async function gitFetchJson(url: string, options: GitHttpOptions = {}): Promise<unknown> {
  ...
  if (response.status === 401) throw new GitError(`authentication failed (HTTP 401) for ${url}`, 'GIT_AUTH_FAILED', { status: 401 })
  ...
  if (response.status === 403) {
    ...
    if (remaining === '0') throw new GitError(`rate limited (HTTP 403) for ${url}`, 'GIT_RATE_LIMITED', { status: 403 })
...
export abstract class GitLocalService extends Service {
...
export abstract class GitPlatformService extends Service {
  constructor(ctx: Context) {
  ...
  abstract getPipelineRun(req: PipelineListRequest & { id: string }): Promise<PipelineRunDetail>
}
