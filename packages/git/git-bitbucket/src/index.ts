...
async function authHeaders(ctx: Context, config: Config): Promise<Record<string, string>> {
  if (!config.username || !config.appPasswordRef) return {}
...
function hostOf(config: Config): string {
  try { return new URL(config.baseUrl ?? 'https://api.bitbucket.org/2.0').host } catch { return 'bitbucket.org' }
}
...
export function apply(ctx: Context, config: Config = {}): () => void {
  ...
  async function bb(method: string, path: string, body: unknown, signal?: AbortSignal): Promise<unknown> {
  ...
  function mapIssue(x: any): Issue {
    return {
      ...
      body: x.content?.raw ?? undefined, labels: (x.labels ?? []).map((l: any) => l.name ?? l),
    }
  }
  ...
  const provider: GitPlatformProvider = {
    ...
    async listPullRequests(req: PullRequestListRequest): Promise<PullRequestSummary[]> {
      const data = await bb('GET', `/repositories/${repoPath(req.owner, req.repo)}/pullrequests?state=ALL&pagelen=${req.limit ?? 30}`, undefined, req.signal) as any
      const values: any[] = data?.values ?? []
      const filtered = values.filter((x) => {
        const state = (x.state ?? '').toUpperCase()
        if (req.state === 'open') return state === 'OPEN'
        if (req.state === 'closed') return state !== 'OPEN'
        return true
      })
    ...
    async listIssues(req: IssueListRequest): Promise<Issue[]> {
      const data = await bb('GET', `/repositories/${repoPath(req.owner, req.repo)}/issues?pagelen=${req.limit ?? 30}`, undefined, req.signal) as any
      const values: any[] = data?.values ?? []
      const open = new Set(['new', 'open', 'on hold'])
      const filtered = values.filter((x) => {
        const state = (x.state ?? '').toLowerCase()
        if (req.state === 'open') return open.has(state)
        if (req.state === 'closed') return !open.has(state)
        return true
      })
    ...
    async listPipelines(req: PipelineListRequest): Promise<PipelineRun[]> {
      const data = await bb('GET', `/repositories/${repoPath(req.owner, req.repo)}/pipelines/?pagelen=${req.limit ?? 30}`, undefined, req.signal) as any
      const values: any[] = data?.values ?? []
    ...
    async getPipelineRun(req: PipelineListRequest & { id: string }): Promise<PipelineRunDetail> {
      ...
      const run = await bb('GET', basePath, undefined, req.signal) as any
      let jobs: PipelineJob[] = []
      try {
        const stepData = await bb('GET', `${basePath}/steps/`, undefined, req.signal) as any
